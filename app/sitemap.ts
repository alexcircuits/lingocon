...
// Regenerate hourly instead of querying the DB on every crawl. This keeps the
// sitemap cheap to serve and, combined with the try/catch fallback below,
// prevents a transient DB issue from turning the sitemap into a 500 (which
// makes Google treat the whole sitemap as unreadable).
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    // ---- Dynamic, DB-backed entries -------------------------------------------
    // Wrapped so a transient DB error degrades to a static-only sitemap instead
    // of a 500 — Google treats a 500 sitemap as the entire sitemap being
    // unavailable, which can drop already-indexed URLs.
    try {
        // ---- Public languages + their content sub-pages -----------------------
        // Only PUBLIC languages with real content are included to avoid
        // thin-content penalties. Each language expands into the section pages
        // that actually have content, plus deep links to individual grammar
        // pages, articles and texts.
        ...
        console.error("[sitemap] Failed to load dynamic entries; serving static pages only:", error);
        return staticPages;
    }
}
