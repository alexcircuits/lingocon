...
export const LanguageNode = memo(({ data, selected }: LanguageNodeProps) => {
  return (
    <Card className={cn(
      "min-w-[180px] p-4 bg-card/80 backdrop-blur-sm cursor-grab active:cursor-grabbing border-2 rounded-2xl transition-colors relative group",
      selected ? "border-primary ring-4 ring-primary/20" : "border-border/60 hover:border-primary/40"
    )}>
      ...
      <div className="flex flex-col items-center text-center gap-2">
        {data.isReadOnly && data.owner ? (
          ...
          <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center -mt-1 mb-1 ring-1 ring-primary/20">
        ...
        <div className="font-bold tracking-tight text-lg leading-tight group-hover:text-primary transition-colors">
          {data.label}
        </div>
